package com.example.uigen.service.llm;
...
public class QwenLlmClient {
    ...
    private String buildUserPrompt(GenerationTask task) {
        ...
        String demoDataInstruction = task.isIncludeDemoData()
                ? """
                Demo data requirement:
                - Add realistic hardcoded demo data in <script setup> (arrays/objects), not empty placeholders.
                - Initial render should show visible populated content (tables/lists/cards/charts should not start empty).
                - Keep data self-contained in component and avoid any network request.
                """
                : """
                Demo data requirement:
                - Do not add extra hardcoded demo data unless user requirement explicitly asks for it.
                """;
        return """
                ...
                Constraints(json): %s
                %s
                """.formatted(task.getComponentName(), task.getPrompt(), constraints, demoDataInstruction);
    }
